...
- **Training**: Fine-tuned on the training set using teacher forcing, optimizing cross-entropy loss: $\alpha^* = \arg \min_\alpha \mathbb{E}_{(x,y)} \left[ - \log p_\alpha (y|x) \right]$
